...
% 
% To make this more concrete, we can form the rotation matrix, $\bm R\in SO(3)$ from a vector of Euler angles, $(\alpha,\beta,\gamma)$, as follows.  Let $\bm e_1=\begin{bmatrix} 1&0&0\end{bmatrix}$ and $\bm e_3=\begin{bmatrix} 0&0&1\end{bmatrix}$ then using the notation from \ref{eqn:angleaxis},
% \[
...
##        [,1]    [,2]    [,3]
## [1,] 0.5263 -0.3746 -0.7633
## [2,] 0.3162  0.9196 -0.2332
## [3,] 0.7893 -0.1186  0.6024
\end{verbatim}
\end{kframe}
\end{knitrout}
...
The four estimates of the central orientation given in the previous section can be plotted by setting \code{estimates show=TRUE}.  The estimators are indicated by shape.  One can also center the data about any observation in $SO(3)$ by setting \code{center=S}.   Typically take \code{center=mean(Rs)}.  
...
rs <- rvmises(50, 0.01)
Rs <- genR(rs)
plot(Rs, center = mean(Rs), show_estimates = T)
\end{verbatim}
\begin{flushleft}\ttfamily\noindent\bfseries\textcolor{errorcolor}{\#\# Error: undefined columns selected}\end{flushleft}\end{kframe}
\end{knitrout}
