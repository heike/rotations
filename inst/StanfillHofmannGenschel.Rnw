...
The \pkg{rotations} package allows users to simulate data from four common choices of symmetric distributions, estimate $\bm{S}$ in \ref{eqn:1} and visualize a sample of rotations.
...
The variety of applications for rotations is echoed by the number of equivalent ways to parameterize them.  We consider the three of the most popular: $3\times 3$ matrices in $SO(3)$, unit quaternions and Euler angles.  

\subsection{Matrix Form}

Given the angle, $r$, and axis, $\bm u$, necessary to define a rotation a $3\times 3$ rotation matrix can be formed by
...
Given a matrix $\bm R\in SO(3)$ it is also useful to be able to find the angle, $r$, and axis, $\bm u$ that form that matrix.  The angle can be found from the fact that $\cos(r)=\frac{1}{2}[\text{tr}(\bm R)-1]$.  The function \code{eangle} will perform this caluclation.
...
\]
Now let $\bm V=2\sin(r)\Phi(\bm u)$ then by the definition of $\Phi(\cdot)$, $u_1=-\bm V_{2,3}/2\sin(r)$, $u_2=\bm V_{1,3}/2\sin(r)$ and $u_3=-\bm V_{1,2}/2\sin(r)$.  The function \code{eaxis} will perform this calculation.
...
\]
where $i,j,$ and $k$ are square roots of -1, i.e. $i^2 = j^2= k^2 = -1$.  We can write $q=(s,v)$ as tuple of the scalar $s$ for coefficient {\bf 1} and vector $v$ for the imaginary coefficients, i.e. $s=x_1$ and $v= (x_2, x_3, x_4)$.
...
The extraction of an axis and angle of rotation from a quaternion is simple based on the above construction.  The function \code{qu} extracts the angle and axis form a rotation matrix then forms a quaternion.
...
All of the functions in \pkg{rotations} will work with any of these representations.  For the remainder of this article, however, we will present our work using the matrix representation.
...
The \pkg{rotate} package allows the user access to four members of the UARS class according to four common distributional models on $r$: the uniform distribution on the sphere, the symmetric matrix Fisher \citep{langevin05, downs72, khatri77, jupp79}, the symmetric Cayley  \citep{Schaeben97, leon06} and the circular von Mises-based distribution \citep{bingham09}. 
