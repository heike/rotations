...
\]
The function \code{genR} takes angles $r$ as arguments, generates an axis uniformly and returns matrices of this form if the \code{space} options is set to \code{SO3}.  

Inversely, for $\bm R\in SO(3)$ one can find the angle, $r$, and axis, $\bm U$ that form that matrix.  The angle can be found from the fact that $\cos(r)=\frac{1}{2}[\text{tr}(\bm R)-1]$.  The function \code{eangle} will perform this caluclation.
