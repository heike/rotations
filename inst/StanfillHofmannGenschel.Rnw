...
The \pkg{rotations} package allows users to simulate data from four common choices of symmetic distributions, estimate $\bm{S}$ in \ref{eqn:1} and visalize a sample of rotations.
...
The variety of applications of rotations is echoed by the number of equivalent ways to parameterize them.
