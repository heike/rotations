...
The function \code{SO3} will form the rotation matrix corresponding to a given angle and axis of rotation.  One can also go the other direction with the functions \code{angle} and \code{axis}, which will return the angle and axis, respectively, for a given roation matrix.  
...
As with the matrix representation, functions to translate an angle and axis into a quaternion and back again are provided.  One can also translate a matrix directly into a quaternion by giving the function \code{Q4} an object of class \code{SO3}.  Similarly, an object of class \code{Q4} can be made into a rotation matrix by using the \code{SO3} function.
...
The final parameterization we consider is \code{Euler angles}.
